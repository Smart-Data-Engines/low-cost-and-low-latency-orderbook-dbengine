...
void IoUringServer::run() {
    ...
    while (running_.load(std::memory_order_relaxed)) {
        ...
        // Sampled **before** submitting, because that is the only moment the number means
        // anything: submitting empties the submission queue, so a sample taken afterwards reads
        // zero whatever the handlers queued. `ready + space_left` is the ring's real capacity
        // rather than the configured depth — the kernel rounds the request up to a power of two,
        // so the configured number is the wrong denominator (#117).
        const unsigned sq_ready = io_uring_sq_ready(ring_);
        const unsigned sq_capacity = sq_ready + io_uring_sq_space_left(ring_);
        engine_->registry().set_gauge("ob_iouring_sq_utilization",
                                      queue_utilization_percent(sq_ready, sq_capacity));

        // Batch submit all new SQEs generated by handlers.
        //
        // The return value used to be discarded, which cost two things. It is the number of SQEs
        // actually submitted, and `ob_iouring_sqe_submitted` was being fed the **completion**
        // count instead — so that metric and `ob_iouring_cqe_processed` were equal by
        // construction, and an operator comparing them to find a backlog compared a number with
        // itself. And a negative return means the entries stayed in the ring: the transport
        // quietly stops making progress, which is #113's shape (a discarded result that only
        // matters when it is not zero) in a different file.
        const int submitted = io_uring_submit(ring_);
        if (submitted < 0) {
            engine_->registry().increment_counter("ob_iouring_submit_errors_total");
            OB_LOG_WARN("io_uring",
                        "io_uring_submit failed: %s — %u prepared entr%s stayed in the ring and "
                        "this loop has made no progress on them",
                        std::strerror(-submitted), sq_ready, sq_ready == 1 ? "y" : "ies");
        }

        // Update io_uring metrics
        engine_->registry().increment_counter("ob_iouring_cqe_processed", count);
        if (submitted > 0) {
            engine_->registry().increment_counter("ob_iouring_sqe_submitted",
                                                  static_cast<uint64_t>(submitted));
        }

        // Completion-queue overflow: a flag the kernel raises while the ring is full, so the
        // counter counts the **episode** and the log says it twice rather than once per pass.
        if (io_uring_cq_has_overflow(ring_)) {
            if (cq_overflow_episode_.begin()) {
                engine_->registry().increment_counter("ob_iouring_cq_overflows");
                OB_LOG_WARN("io_uring",
                            "completion queue overflowed: the kernel is holding completions it "
                            "could not deliver, so this loop is behind the ring. Further passes "
                            "are counted in ob_iouring_cq_overflows, not logged");
            }
        } else if (const uint64_t passes = cq_overflow_episode_.end(); passes > 0) {
            OB_LOG_WARN("io_uring", "completion queue overflow cleared after %llu loop pass(es)",
