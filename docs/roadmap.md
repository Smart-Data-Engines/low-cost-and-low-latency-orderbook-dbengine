...
Verified by [the full CI run for PR #130](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/34905051315),
on the tree containing #126 to #133 — which is **all four** of #112's loops, plus the three items
that closing it produced. Runtimes below are from GitHub's `ubuntu-24.04` runners except where a row
says otherwise, not the machine-B performance baseline above.
