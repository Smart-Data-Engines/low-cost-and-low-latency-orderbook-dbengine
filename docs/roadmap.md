...
### 35. Refactor the argument parser to stop mutating the loop counter
...
### 38. Reproducible comparative benchmarks
...
### 40. Client libraries: Rust and Go
...
### 42. Worked example: live market data ingestion
...
### 43. Time-bucketed aggregation in the query language
...
### 46. Zone maps and columnar indexes
...
### 49. NUMA awareness and thread pinning
...
### 51. Shared-memory transport for local clients
...
### 54. Multi-node cluster tests in CI
...
### 56. Complete the anti-entropy implementation
...
| **P1** | Reproducible comparative benchmarks (#38) | L | Makes the performance claim verifiable by a reader instead of asserted |
| **P1** | Authentication and TLS (#29) | L | The single blocker to production adoption |
| **P2** | Argument parser refactor (#35) | S | Closes 27 static-analysis findings; without it every PR adding a CLI flag needs a manual review-thread resolution |
| **P2** | CI hardening with sanitizers (#36) | S | Strong quality signal, low cost, catches real concurrency bugs |
| **P2** | Configuration file (#31) | S | Ops ergonomics |
| **P2** | Documentation site (#39) | M | Lowers evaluation friction |
| **P2** | Release engineering + PyPI wheels (#41) | S | `pip install` is the shortest path to a first user |
| **P3** | Time-bucketed aggregation (#43) | L | The most-requested analytical capability for this data |
| **P3** | Arrow output (#45) | M | Near-zero integration cost for analytics teams |
| **P3** | Backup and restore (#33) | M | Table stakes for a database |
| **P4** | Chaos testing (#53) | L | Do this once there are users whose data can be lost |
| **P4** | Performance frontier (#48-48) | varies | Proves the bespoke-engine claim; pick one and write it up |
...
- **Anti-entropy is a scheduler with no reconciliation** (#56). The spec task is marked complete and
...
- **The README advertises streaming subscriptions** that are not verified to exist in the current
  wire protocol (#44): implement it or correct the claim.
