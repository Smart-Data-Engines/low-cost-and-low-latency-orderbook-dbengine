...
- Effort: S | Impact: a `SNAPSHOT` answers with a book that never existed at its time whenever a
...
- Effort: S | Impact: the documented way to reconstruct a book at a point in time answers nothing
  over TCP, and says `OK`
...
**#165 is open, and a P0**, and **#167 and #168 are open P1s** — the mechanical list is the `Open:`
line below; read it there rather than trusting this paragraph, which is prose and has been wrong
about this before. **#165's first part is done**: the index is per symbol and in width tiers, so a
...
uptime. **#167 and #168** are `SNAPSHOT`: over the wire it has answered with no columns since #139,
and in any mode it keeps the last row delivered for a level rather than the latest at or before its
time. **#166 was a P0
...
**Open: #165, #167, #168.** Every other item above #58 is marked closed, and
...
| **P1** | `SNAPSHOT` answers its columns over the wire (#167) | S | Since #139 it answers `OK` with an empty header and empty rows |
| **P1** | `SNAPSHOT` keeps the latest row at or before its time (#168) | S | It keeps the last one delivered, so a correction for an earlier instant replaces the book it came after | |
...
Verified by [the full CI run for PR #178](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/35921254262),
on the tree whose flush tick syncs, drains and deletes without the engine's lock (#164).

**Against PR #176's run (1249 and 364) C++ is +22 and integration +8, and both reconcile to
files.** The twenty-two C++ tests are #164's: nine `ChunkedQueue.*` in the new
