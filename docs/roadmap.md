...
**Open: #160.** Every other item above #58 is marked closed, and
...
#160 is closed. #121, the last question among them, was answered by bounding how far a peer's clock
...
Verified by [the full CI run for PR #172](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/35850783266),
on the tree carrying `boost` as the default, sized to the machine (#158).

**Against PR #169's run (1182 and 330) C++ is +46 and integration is +10, over three merges**, and
both deltas reconcile to files. #155 (PR #170) added twelve C++ — six in `tests/test_write_batch.cpp`,
four `WalBatch` in `tests/test_wal.cpp`, one for the read loop's order and one for the authentication
gate a held write passes — and eight integration, seven in the new
`tests/integration/test_write_batches.py` and one in `test_auth.py`. #156 (PR #171) added twenty-five
C++, twenty-three in the new `tests/test_machine.cpp` and two in `tests/test_cli_config.cpp`; #157
rewrote the sustained-insert test rather than adding one. #158 (PR #172) added twelve C++ and
**renamed three** whose names said `eco` was the default, and two integration in the new
`tests/integration/test_io_profile.py`. The row before this one cited PR #169 for three merges, which
is the lag a table maintained by hand has; the numbers never disagreed with their citation.
