...
beside.** `ReplicationClient::save_state()` opens `repl_state.txt` with `O_TRUNC` and writes it
**in place**, so a replica killed between the truncate and the write leaves an empty file - which
`load_state()` reads as a position with no identity, that is as "wipe the store" (#101), and which
also forgets the epoch #103 made that file the only keeper of. That window needs no power cut - a
kill at the wrong moment is enough, and the file is rewritten every ten seconds - and how wide it is
has not been measured. Two more of the same
shape: `snapshot_manifest.json` is written to a temporary and renamed with neither the file nor the
directory synced, and the files a snapshot install puts in place are not synced before the state
that points at them is saved.

The instrument exists (`write_file_atomically()` in `include/orderbook/durable_file.hpp`, which #160
uses for `wal_identity`), and a measurement is the first task: kill a replica inside
`save_state()` - the injector can hold the write open - and read what the next start does.

- Effort: S | Impact: a replica killed at the wrong moment wipes its store, resyncs from scratch and
  forgets its epoch; after a power cut, a snapshot install can be half on the disk
...
**No P0 is open; #162 is** — the mechanical list is the `Open:` line below; read it there rather
than trusting this paragraph, which is prose and has been wrong about this before. It is a replica
state file rewritten in place, which a kill at the wrong moment turns into a full resync and a
forgotten epoch. **#160 and #161 were P0s and are closed**: segment files were never synced, so a
...
**Open: #162.** Every other item above #58 is marked closed, and
...
Read that as narrowly as it is written. It says every defect **that has been filed** above #58 but
#162 is closed. #121, the last question among them, was answered by bounding how far a peer's clock
...
Verified by [the full CI run for PR #173](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/35858513780),
on the tree carrying a checkpoint that claims what its flush drained (#159).

**Against PR #172's run (1228 and 340) C++ is +8 and integration is +5**, and both reconcile to
