...
Verified by [the full CI run for PR #134](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/34927916879),
on the tree containing #126 to #133 with **#132 now fixed rather than pinned** — which is every
loop in `src/` guarding one iteration at a time, and a node whose registry entry is confirmed gone
writing it again. Runtimes below are from GitHub's
