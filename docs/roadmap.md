...
Verified by [the full CI run for PR #139](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/34947630036),
on the tree containing #126 to **#135 with #130 closed** — which is the page's **last open
defect**: every loop in `src/` guards one iteration at a time, a node whose registry entry is
confirmed gone writes it again, the peer registry addresses the endpoint that answered, and a won
promotion that cannot finish keeps trying instead of holding the role. Runtimes below are from GitHub's
