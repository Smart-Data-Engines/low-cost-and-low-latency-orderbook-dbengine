...
Verified by [the full CI run for PR #169](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/35796568217),
on the tree carrying a WAL rotation that no longer strands or refuses a written record (#153, #154).

**Against PR #168's run (1181 and 327) C++ is +1** — `WalRotation.ANextFileThatCannotBeCreatedIsOpenedOnceItCan`
in `tests/test_wal.cpp` (#154) — **and integration is +3**, the three tests
`tests/integration/test_storage_faults.py` gained for a failed sync while rotating, a refused marker
and a next file that cannot be opened. Both deltas reconcile to files, and nothing else moved.
