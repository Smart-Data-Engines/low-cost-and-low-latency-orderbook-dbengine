...
Verified by [the full CI run for PR #125](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/34874031672),
on the tree containing #126, #127, #128 and #129. Runtimes below are from GitHub's `ubuntu-24.04`
runners except where a row says otherwise, not the machine-B performance baseline above.

The citation and the counts have to name the **same tree**, which is why this paragraph moved with
#129 rather than a commit later: for one commit it cited PR #124's run beside a C++ count of 1083,
and #124's tree had 1082. One test's worth of disagreement is exactly how a table stops being a
measurement — and this row's whole purpose is to be one.
