...
Verified by [the full CI run for PR #137](https://github.com/Smart-Data-Engines/low-cost-and-low-latency-orderbook-dbengine/actions/runs/34939049434),
on the tree containing #126 to **#135** — every loop in `src/` guarding one iteration at a time, a
node whose registry entry is confirmed gone writing it again, and the peer registry addressing the
endpoint that answered rather than the first one configured. Runtimes below are from GitHub's
