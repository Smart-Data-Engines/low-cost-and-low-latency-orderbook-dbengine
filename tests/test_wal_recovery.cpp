...
TEST(WalRecovery, RecordsAlreadyCoveredByASegmentAreNotReplayed) {
    ...
    // Re-append the second batch with no checkpoint after it: exactly the on-disk
    // state a crash between writing those segment files and appending the checkpoint
    // leaves behind. The rows are already durable; the WAL cannot know it.
    {
        ob::WALWriter writer(tmp.path, 512ULL << 20, ob::FsyncPolicy::EVERY);
        for (int i = 2; i < 4; ++i) {          // a strict subset: the WAL is truncated
            ob::DeltaUpdate delta{};            // to the replica-confirmed position, so
            std::strncpy(delta.symbol, "CRASH", sizeof(delta.symbol) - 1);   // its tail
            std::strncpy(delta.exchange, "EX", sizeof(delta.exchange) - 1);  // can hold
            delta.sequence_number = static_cast<uint64_t>(i + 1);            // fewer rows
            delta.timestamp_ns    = kSecond + static_cast<uint64_t>(i) * 1'000'000ULL;
            delta.side            = ob::SIDE_BID;
            delta.n_levels        = 1;

            ob::Level level{};
            level.price = 140'000 + i;
            level.qty   = 100 + static_cast<uint64_t>(i);
            level.cnt   = 1;
            level._pad  = 0;
    ...
    // A second, clean restart: the merge of a re-flushed segment is refused, but the
    // refusal comes after its files were already rewritten in place, so the damage
    // only surfaces on the next open_existing() — one restart late.
    ...
    EXPECT_EQ(after_recovery, 8)
        << after_recovery << " rows after recovery: records whose timestamps a segment "
           "already covers were replayed on top of it, rewriting a durable segment "
           "from whatever the WAL tail happened to hold. This is the crash window no "
           "checkpoint can describe, and the timestamp guard is what closes it";
    EXPECT_EQ(after_restart, 8)
        << after_restart << " of 8 rows after a further clean restart: replaying over a "
           "durable segment rewrote it with fewer rows than it held, which is data loss, "
           "not duplication";
}
